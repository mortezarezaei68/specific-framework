...
public static class RiderProducerExtensions
{
    ...
    private static void GetConfiguration(Assembly[] assemblies, Type kafkaProducer, out ProducerConfig? producerConfig,
        out Delegate? @delegate, out string topicName)
    {
        ...
        topicName = kafkaProducer.BaseType.GetGenericArguments()[0].Name.Underscore();
    ...
    private static void ProducerConfigure(this IRiderRegistrationConfigurator riderConfiguration, Type kafkaProducer, string topicName,
        ProducerConfig? producerConfig, Delegate? @delegate = null)
    {
        typeof(RiderProducerExtensions).GetMethod(nameof(RiderExtension))
            ...
            {
                riderConfiguration,
                string.IsNullOrWhiteSpace(topicName)? kafkaProducer.Name : topicName,
                producerConfig,
                @delegate
            });
    }
        ...
        return assemblies
            .SelectMany(listOfAssemblies => listOfAssemblies.GetTypes()).FirstOrDefault(t =>
                t.GetInterfaces().Any(type => type.IsGenericType &&
                                              type.GetGenericTypeDefinition() == (typeof(KafkaProducerConfiguration<>)) && 
